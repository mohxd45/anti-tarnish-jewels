...
- [ ] Phase 2 Security: Secure Checkout & Coupon Validation Backend
  - [ ] Implement backend validation API Route (`/api/checkout`)
  - [ ] Use `firebase-admin` for securely interacting with Firestore
  - [ ] Fetch product prices on backend
  - [ ] Validate coupons on backend
  - [ ] Secure Firestore rules for `orders` and `coupons`
