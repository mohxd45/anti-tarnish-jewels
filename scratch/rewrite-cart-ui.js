...
export default function CartPage() {
  const cart = useCart();
  const { items, dispatch, subtotal, total, discount, appliedCoupon, removeCoupon } = cart;
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {items.map(item => (
              <div key={item.id} className="glass-dark p-4 md:p-6 rounded-2xl flex gap-4 md:gap-6 items-center shadow-sm relative border-[var(--glass-border)]">
                <Link href={\`/product/\${item.slug || item.id}\`} className="flex-shrink-0">
                  <div className="w-20 h-24 md:w-24 md:h-28 bg-[var(--ivory)] rounded-xl overflow-hidden relative">
                    <Image src={getOptimizedImageUrl(item.image, 200)} alt={item.name} fill className="object-cover" />
                  </div>
                </Link>
                <div className="flex-grow">
                  <p className="text-[var(--gold-dark)] text-[10px] md:text-xs font-semibold tracking-wider uppercase mb-1">{item.category}</p>
                  <Link href={\`/product/\${item.slug || item.id}\`}>
                    <h3 className="font-display text-lg md:text-xl text-[var(--ink)] font-semibold line-clamp-1 hover:text-[var(--rose)] transition-colors">{item.name}</h3>
                  </Link>
                  <p className="text-[var(--stoneGray)] text-sm mb-3">₹{item.price}</p>
              ...
              <div className="mb-6">
                {!appliedCoupon ? (
                  ...
                  <div className="flex items-center justify-between bg-[var(--pink-100)] px-4 py-2.5 rounded-xl border border-[var(--pink-200)]">
                    <span className="text-sm font-medium text-[var(--success)]">✨ {appliedCoupon.code} Applied!</span>
                    <button onClick={removeCoupon} className="text-xs text-[var(--stoneGray)] hover:text-red-500 underline">Remove</button>
                  </div>
                )}
                {couponMsg && !appliedCoupon && <p className="text-xs text-red-500 mt-2">{couponMsg}</p>}
              </div>
