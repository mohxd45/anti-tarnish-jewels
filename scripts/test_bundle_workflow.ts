...
async function runTest() {
  ...
  const fetchedBundle = doc.data() as any;
  const eligibleItems = (fetchedBundle?.independentBundleItems || []).filter((i: any) => i.active && i.stock > 0);
  console.log("Eligible items for storefront:", eligibleItems.map((i: any) => i.name));
  ...
  await db.runTransaction(async (t) => {
    ...
    for (const finalItem of finalItems) {
      for (const pData of finalItem.selectedProducts) {
        const itemIdx = independentItems.findIndex((i: any) => i.id === pData.productId);
  ...
  const ring = postItems.find((i: any) => i.id === "item-1");
  const neck = postItems.find((i: any) => i.id === "item-2");
  ...
  try {
    await db.runTransaction(async (t) => {
      ...
      const itemIdx = independentItems.findIndex((i: any) => i.id === fakeId);
      if (itemIdx === -1) {
