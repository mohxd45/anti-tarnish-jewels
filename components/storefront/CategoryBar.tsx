    ...
    async function load() {
      try {
        ...
        const navCategories = [
          ...
          ...activeCategories.map(c => ({ name: c.name, slug: c.slug }))
        ];
        setCategories(navCategories);
      } catch (err) {
        console.error("Failed to load categories for CategoryBar:", err);
        // Fallback to preserve All Jewellery if Firebase fails
        setCategories([{ name: "All Jewellery", slug: "all" }]);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="w-full bg-white/50 backdrop-blur-md border-b border-stone-100 shadow-sm block lg:hidden pb-3">
      <div className="mx-4 mt-2 bg-gradient-to-r from-[#FFF5F8] to-[#FFF9E6] border border-[#E9C8A1]/40 rounded-xl shadow-[0_2px_12px_rgba(184,149,94,0.06)] overflow-hidden">
        <div className="flex gap-2.5 overflow-x-auto px-3 py-2.5 scrollbar-hide snap-x snap-mandatory [&::-webkit-scrollbar]:hidden items-center">
          {loading ? (
            // Minimal neutral loading state to prevent layout shift and flash of old categories
            [...Array(6)].map((_, i) => (
              <div key={i} className="flex items-center justify-center px-4 h-[34px] rounded-lg bg-white/50 animate-pulse shrink-0 border border-transparent w-24"></div>
            ))
          ) : (
            categories.map((cat) => {
              const isActive = 
