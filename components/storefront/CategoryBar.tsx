...
export function CategoryBar() {
  ...
  return (
    <div className="sticky top-[64px] z-40 bg-[#FFF0F5]/95 backdrop-blur-md border-b border-[#B8955E]/20 block md:hidden shadow-sm">
      <div className="flex gap-3 overflow-x-auto px-4 py-3 scrollbar-hide snap-x snap-mandatory [&::-webkit-scrollbar]:hidden">
        {categories.map((cat) => {
          ...
          return (
            <Link 
              key={cat.name} 
              href={href}
              className={`flex items-center gap-2 px-5 h-[50px] rounded-full border transition-all duration-300 snap-start shrink-0 ${
                isActive 
            ...
            >
              <Icon className={`w-[18px] h-[18px] ${isActive ? "text-white" : "text-[#B8955E]"}`} strokeWidth={isActive ? 2.5 : 2} />
              <span className={`text-[14px] whitespace-nowrap tracking-wide ${isActive ? 'font-semibold' : 'font-medium'}`}>
                {cat.name}
              </span>
            </Link>
          );
