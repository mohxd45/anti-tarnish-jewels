...
export function CartRewardTracker({ subtotal }: CartRewardTrackerProps) {
  ...
  return (
    <div className="bg-brandCardBg p-4 border-b border-brandBorder/30 rounded-b-2xl shadow-sm mb-3 mx-4 mt-2">
      <p className="mb-3 text-[15px] font-semibold text-brandEspresso text-center max-w-[280px] mx-auto leading-relaxed">
        {message}
      </p>
      
      {/* Tracker Bar */}
      <div className="relative w-full h-2 bg-stone-100 rounded-full mb-4 overflow-hidden shadow-inner border border-stone-200">
        <motion.div 
        ...
        {milestones.map((milestone, idx) => {
          ...
          return (
            ...
            >
              {/* Circle Icon */}
              <div 
                className={`flex items-center justify-center w-[42px] h-[42px] shrink-0 rounded-full shadow-sm transition-colors ${
              ...
              <div className="flex-1 flex justify-between items-center">
                <span className={`text-[15px] font-semibold ${isReached ? 'text-brandEspresso' : 'text-stone-500'}`}>
                  {milestone.label}
                </span>
                <span className={`text-[15px] font-bold ${isReached ? 'text-brandGoldDeep' : 'text-stone-400'}`}>
