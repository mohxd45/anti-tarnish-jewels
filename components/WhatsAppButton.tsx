...
export function WhatsAppButton() {
  ...
  // Hide button on admin routes, disabled setting, or if no number configured
  if (pathname.startsWith("/admin") || !showButton || !whatsAppNumber) {
    return null;
  }

  // Debug log in development to verify rendering globally
  if (process.env.NODE_ENV === "development") {
    console.log("WhatsApp floating button rendered", { pathname, windowWidth: typeof window !== "undefined" ? window.innerWidth : 0 });
  }
  ...
  return (
    <a
      ...
      className="fixed right-4 bottom-[calc(90px+env(safe-area-inset-bottom))] md:right-6 md:bottom-6 z-[99999] flex items-center justify-center gap-2 rounded-full bg-[#25D366] text-white w-[56px] h-[56px] md:w-auto md:h-auto md:px-5 md:py-3 shadow-[0_4px_14px_rgba(37,211,102,0.4)] border-2 border-white hover:bg-[#20ba56] hover:scale-105 transition-all duration-300 select-none cursor-pointer"
