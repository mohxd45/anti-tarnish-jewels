...
export function ProductCard({ product }: { product: Product }) {
  ...
  const badge =
    isSale ? { label: "Sale", cls: "bg-pink-500 text-white" } :
    product.isBestSeller ? { label: "Bestseller", cls: "clay-badge px-3 py-1 text-pink-900" } :
    ...
    <div className="glass-premium card-hover hover-tilt group flex flex-col overflow-hidden rounded-2xl shine-sweep">
      ...
      >
        ...
        <button
          ...
          className={`glass absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full transition hover:text-pink-700 ${isWishlisted ? 'text-red-500' : 'text-pink-600'}`}
      ...
      <div className="flex flex-1 flex-col gap-2 p-3 sm:p-4">
        ...
        <div className="mt-auto flex items-baseline gap-2 pt-1">
          {isSale ? (
            <>
              <span className="text-base font-bold text-pink-900">₹{product.salePrice}</span>
              <span className="text-xs text-pink-600 line-through">₹{product.regularPrice}</span>
            </>
          ) : (
            <span className="text-base font-bold text-pink-900">₹{product.salePrice}</span>
          )}
