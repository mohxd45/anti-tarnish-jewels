  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-serif font-semibold text-adminSidebar tracking-tight">Categories</h1>
          <p className="text-adminMuted mt-1">Organize your catalog collections</p>
        </div>
      ...
      {loading ? (
        ...
        <AdminCard className="p-12 text-center shadow-sm">
          <h3 className="text-xl font-serif text-adminSidebar">No Categories Configured</h3>
          <p className="text-adminMuted text-sm mt-2">Click the button above to define your first shopping category.</p>
        </AdminCard>
      ) : (
        <div className="grid gap-4 grid-cols-[repeat(auto-fill,minmax(240px,1fr))]">
          {categories.map((c) => (
            <div key={c.id} className="p-4 flex flex-col gap-3 rounded-2xl border border-adminBorder bg-white hover:bg-adminBg/50 transition-colors shadow-sm">
              <div className="aspect-[4/3] rounded-lg bg-adminBg overflow-hidden relative border border-adminBorder">
                {c.imageUrl ? (
                ...
                {c.bannerUrl && (
                  <div className="absolute inset-x-0 bottom-0 h-1 bg-adminGold/40" title="Has Banner" />
                )}
              </div>
              <div className="flex items-start justify-between gap-2 mt-1">
                <div className="min-w-0">
                  <h3 className="font-serif text-lg font-medium leading-tight truncate text-adminSidebar">{c.name}</h3>
                  <p className="text-xs text-adminMuted mt-0.5">Priority #{c.priority}</p>
                </div>
                <StatusBadge status={c.isActive ? "Active" : "Inactive"} />
              </div>
              
              <div className="text-[10px] text-adminMuted line-clamp-1 min-h-[15px]">
                {c.subcategories?.length > 0 ? c.subcategories.join(", ") : "No subcategories"}
              </div>

              <div className="flex gap-1.5 mt-auto pt-2">
                <Button size="sm" variant="outline" onClick={() => openEdit(c)} className="flex-1 rounded-xl h-8 text-xs border-adminBorder text-adminSidebar hover:bg-adminBg">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex justify-end animate-in fade-in">
          ...
          <aside className="relative h-full w-[400px] max-w-[100vw] border-l border-adminBorder bg-white shadow-2xl overflow-y-auto flex flex-col animate-in slide-in-from-right">
            ...
            <div className="flex justify-between items-center border-b border-adminBorder p-5 bg-adminBg backdrop-blur-sm">
              <div>
                <span className="text-[10px] uppercase tracking-widest text-adminMuted font-semibold">Category Builder</span>
                <h3 className="text-xl font-serif font-semibold text-adminSidebar mt-1">
                  {editingCategory.id ? "Edit Category" : "Add Category"}
                </h3>
              </div>
              <button type="button" onClick={() => setEditorOpen(false)} className="rounded-full p-2 text-adminMuted hover:bg-white transition-colors">
                <X size={18} />
              </button>
            </div>
              ...
              <form id="category-form" onSubmit={handleSave} className="space-y-5">
                ...
                <Field label="Subcategories (Comma separated)">
                  <Textarea
                    ...
                    className="rounded-xl resize-none border-adminBorder bg-white text-adminSidebar"
                  />
                </Field>
                ...
                <Field label="Category Icon">
                  <div className="flex gap-2 items-center">
                    {(imageFile || editingCategory.imageUrl) && (
                      <div className="h-10 w-10 shrink-0 rounded-lg bg-adminBg border border-adminBorder overflow-hidden">
                    ...
                    <Input
                      ...
                      className="rounded-xl border-adminBorder bg-white text-adminSidebar"
                    />
                    <label className="shrink-0 rounded-xl bg-adminBg hover:bg-adminBg/80 px-3 py-2 text-xs font-semibold cursor-pointer border border-adminBorder transition-colors text-adminSidebar">
                ...
                <Field label="Category Banner (Optional)">
                  <div className="flex gap-2 items-center">
                    {(bannerFile || editingCategory.bannerUrl) && (
                      <div className="h-8 w-16 shrink-0 rounded bg-adminBg border border-adminBorder overflow-hidden">
                ...
                <div className="pt-2">
                  <label className="flex items-center gap-3 text-sm cursor-pointer select-none border border-adminBorder bg-adminBg p-3 rounded-xl hover:bg-adminBg/80 transition-colors">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-adminMuted">Show in shop navigation</div>
                    </div>
                  </label>
                </div>
            ...
            <div className="border-t border-adminBorder p-5 bg-white backdrop-blur-sm flex justify-end gap-3 mt-auto">
              <Button variant="outline" onClick={() => setEditorOpen(false)} className="rounded-full border-adminBorder text-adminSidebar hover:bg-adminBg">Cancel</Button>
              <Button form="category-form" type="submit" disabled={uploading} className="rounded-full bg-adminRose text-white hover:bg-adminRose/90 border-none min-w-[120px] shadow-md">
